...
public class LevenstheinDistance {
    ...
    public static double normalizedLevenstheinDistance(CharSequence s1, CharSequence s2) {
     ...
     * @result levenshtein distance between s1 and t2, or {@link #MAX_DISTANCE}
     *      if the distance is >= MAX_DISTANCE
     */
    public static int levenshteinDistanceK(CharSequence s1, CharSequence s2) {
        ...
        int ignore = MAX_DISTANCE; // the original algorithm contains MAX_DISTANCE + 1, but this shouldn't break anything
        ...
        final int lSize = (((MAX_DISTANCE > length2) ? length2 : MAX_DISTANCE) - lengthDifference) / 2; // left space for insertions
        final int rSize = lSize + lengthDifference; // right space for deletions, rsize >= lsize (rsize == lsize iff r == 0)
          ...
          for (int j = bl; j <= br; j++) {
            if (j == 0) { // postponed part of initialization
                ...
                if (str1.charAt(im1) == str2.charAt(jm1)) {
                    ...
                    int b = (stripSizem1 == jv) ? ignore : distance[lastRow + jv + 1]; //insertion
