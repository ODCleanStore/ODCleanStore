...
class Insert implements Runnable {
    ...
    private static final int UNKNOWN_HOST_ERROR_CODE = 128;
    private static final int CONNECTION_ERROR_CODE = 129;
    private static final int RESPONSE_TIMEOUT_ERROR_CODE = 130;
     ...
     */
    public void run(String user, String password, Metadata metadata, Reader payloadReader, Reader payloadReaderForSize)
        ...
        try {
            ...
            if (isExceptionDuringResponse) {
                throw new InsertException(CONNECTION_ERROR_CODE, "Connection error", "Connection error");
            }
            if (responseID != 0) {
                throw new InsertException(responseID, responseMessage, responseMoreInfo);
            }
            if (response.getResponseCode() < HttpURLConnection.HTTP_OK /*200*/
                    || response.getResponseCode() >= HttpURLConnection.HTTP_MULT_CHOICE /*300*/) {
                throw new InsertException(CONNECTION_ERROR_CODE, "Connection error", "Connection error");
            }
            ...
            throw new InsertException(RESPONSE_TIMEOUT_ERROR_CODE, "Connection error", "Response timeout");
        } finally {
            if (request != null) {
    ...
    private Socket createClientConnection() throws InsertException {
        try {
            ...
            throw new InsertException(UNKNOWN_HOST_ERROR_CODE, "UnknownHost", "UnknownHost");
        } catch (IOException e) {
            throw new InsertException(CONNECTION_ERROR_CODE, "Connection error", "Connection error");
        }
    }
    ...
    public void run() {
        try {
            ...
            if (response.getResponseCode() ==  HttpURLConnection.HTTP_INTERNAL_ERROR /*500*/
