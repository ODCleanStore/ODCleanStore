...
where \code{\$\$graph\$\$} in \code{GRAPH \$\$graph\$\$} is a place holder for name of the graph being currently processed and can be used for subqueries that need to be enclosed in \code{GraphGraphPattern}\footnote{\url{http://www.w3.org/TR/rdf-sparql-query/\#rGraphGraphPattern}}.

Note that when there is no subquery, the \code{\$\$graph\$\$} placeholder is optional and it is not necessary to use the placeholder at all.
...
The output webservice configuration covers default policies for data aggregation. See section \ref{sec:outputWS}.
...
\label{sec:transformerManagement}
	...
	\fieldtable
	{
		...
		\hline
		required & Full classname & Name of the class implementing the {transformer}. Note that it isn't possible to edit this value at a later time.
	}

	If \emph{JAR path} is set to \quot{.} then it is handled as a special case and \emph{Full classname} is treated as a built-in transformer.
...
\vspace*{0.2\textheight}
	...
	\item Choose \code{Pipelines} from the main menu
...
\subsection{Quality Calculation}
\label{sec:outputWSQuality}
As stated in the previous sections, an \aggrq estimate for each triple in the result is part of the query results. The quality is expressed as a number from interval [0,1] where 0 means lowest quality and 1 highest quality. 

The \aggrq estimate is done for each result quad and is based on several factors based on real-world scenarios. The factors include quality scores of the source named graphs (as calculated by the Quality Assessor transformer, Section \ref{sub:qualityAssessment}), number of graphs that agree on a value, and the difference between a value and other (conflicting) values.
...
\begin{itemize}
    \item \QA scores of graphs that the value was selected or calculated from are taken. Depending on the aggregation method, their average or maximum is used as the initial score.
    \item Differences with other conflicting values are taken into consideration. The more conflicting values differ, the more the quality is decreased. This step can be turned off by setting the \code{multivalue} parameter to false (\code{0}).
    \item If there are multiple sources that agree on exactly the same value, the quality is increased.
\end{itemize}

This is a very crude description of the algorithm. You can find it explained in detail in section \quot{Quality and Provenance Calculation} of \refprogrammersguide.
