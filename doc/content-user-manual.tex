...
The URIs generated in place of blank nodes have form \varcode{prefix}\varcode{randomUUID}-\varcode{node number}. The prefix may be given in \code{Configuration} field of the transformer instance as \quot{\code{uriPrefix=}\varcode{URI prefix}} on a single line. If the prefix is not specified, the default value \code{odcs:genResource/} is used.
