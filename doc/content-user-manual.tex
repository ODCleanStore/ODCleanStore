...
ODCleanStore is planned to be deployed together with the Data Acquisition module represented by project Strigil\footnotemark[\thefootnote] which would feed up-to-date data to ODCleanStore. However, thanks to the use of standard formats for communication with the input/output webservices, ODCleanStore can be deployed together with any third-party application for data feeding or consuming.
...
See Section \ref{sec:inputProcessing} for more information about how the inserted data is processed and stored.
...
\begin{tabular}{|l|l|l|}
	...
	\hline
	\code{payload} & inserted data serialized as RDF/XML or TTL & \vartext{string} \\
	\hline
...
UUID doesn't change during the shole message transfer nor in case of a~repeated request after an exception.
...
\begin{tabularx}{\textwidth}{|l|c|X|}
	...
	\hline
	OTHER\_ERROR & 7 & Other error; when a~new transmission with the same \code{uuid} as the current \code{uuid} is started before the current transmission finishes, OTHER\_ERROR is thrown; the new transmission will continue \\
	\hline
...
Table \ref{tbl:requestFormatUK} lists (either GET or POST) parameters than can be used with the URI and keyword queries.
...
Data originating from a single request to Input Webservice can be stored in several named graphs.

RDF data given in the \code{payload} parameter are stored in one named graph (\code{payload} graph). If \code{provenance} RDF metadata are given, they are stored in another named graph (\code{provenance} graph). Other metadata (such as the source of data, timestamp, etc.) are stored in yet another named graph (\term{metadata graph}). In addition, transformers in the respective pipeline may add more related RDF data to one or more named graphs, e.g. results of quality assessment, or mappings for resources in \code{payload} (\term{attached graphs}). 
...
\begin{glossarylist}
	...
	\item[Named graph score (\code{odcs:score})] Quality of a~single (\code{payload}) named graph estimated by the Quality Assesment component and stored in the database, expressed as a~number from interval [0,1]
