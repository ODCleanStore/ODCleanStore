...
All our custom validators extends the \code{CustomValidator} class, which handles the error message propagation so that the validators can focus just on the validation process.
...
\end{itemize}
    %dalsi policies (krome conflict resolution policies):
        %(provenance) policies, tedy napr. mit možnost vyrazne snizit skore pro data pochazejici ze zdroje http://example.com - viz pravidlo "Distrust data coming from a datasource http://example.com". Byl bych pro nasledujici modifikatory v pravidlech: trust/distrust (znacny bonus/postih), prioritize/deprioritize (mensi bonus/postih), filter (jen dana data/jen ostatni data) - obecne pravidla spojena s kontextem dat. Dalsi priklad: "Distrust data older than 1 year (created at least one year ago)"
        %content policies ("Distrust data with completeness less than ...") - obecne pravidla spojena s content analyzami v EL.
        %rating policies ("Distrust least popular sources") - obecne pravidla spojena s hodnocenim popularity zdroju, pripadne jejich autoritativnosti (ma smysl az po vytvoreni patricnych analyz zdroju)

    %podpora CR pro SPARQL odtazy
    %stránkování výsledků
    %razeni vysledku dotazu na klicove slovo dle relevance
    %moznot omezit vysledky na resources s danym rdf:type - v URI search pro hledane URI, v KW search pro subjecty ve vysledku
    %možnost dotázat se na všechny resources daného typu (rdf:type)

%Conflict Resolution

    %hodnocení kvality i podle cratora nebo jiného agributu
    %uživatel může nastavi svou (ne)důvěru v publishera
    %v kvalitě zohledňovat aktuálnost dat
    %agregace TOP-K (K nejlepších hodnot), obecně parametry agregačních funkcí (např. oddělovač u CONCAT)

%Engine

    %detekce identické aktualizace
    %Nabizet data zpracovana pipeline a surova data, tak jak dorazila od scraperu. Mit moznost kdykoliv pri rerunu pouzit surova data. (pokud si je zprasim)
        %Tedy data co se poslou na pipeline taky ulozit vedle ve spinave a propojit s danymi daty poslanymi do pipeline, tahle data tam zustanou.
        %Viz viceropipeline, s tim, ze puvodni data jsou ta, co projdou prazdnou pipeline.
    %? Podporovat vicero pipelines pro stejna data (~ vicero konfiguraci, profilu)
        %jedna nechava surova data, druha konzervativni (jen QA, simple linkers, basic DN), tretia agresivni
        %Realizace: Je mozne vytvorit nekolik pipeline {name}-1, {name}-2 .. Vstupni WS posle data do vsech pipeline, ulozi se pak jako alternativni grafy v ciste databazi - konzument ma moznost specifikovat, ze ho zajimaji jen konzertvativni data.
        %Conflict resolution - ma to byt soucasti profilu nebo je to nastaveni ortogonalni?

%Custom Transformers

    %možnost konfigurace, kdy se má krabička spouštět (globálně/podle zdroje/podle ontologie - jako u ostatních policies)
    %možnost nakonfigurovat krabičky pro jmenný graf při vkládání skrze vstupní WS

%Webový frontend

    %uživatel v roli ADM může editovat libovolnou pipeline i skupinu pravidel
    %každý má možnost vytvořit vlastní kopii (i cizí) skupiny pravidel (dále fork); takové skupině musí přířadit unikátní název
    %možnost forku pro jednotlivá pravidla
    ...
    %viditelnost skupiny pravidel/pipeline může být označena jako private nebo public
    %notifikace a příjímání změn ve skupině pravidel
    ...
    %uživatel bude mít k dispozici předpřipravené instance transformerů (šablony), které může vložit do své pipeliny
    ...
    %zavést skupiny uživatelů, přidělovat oprávnění k editaci pipelines/pravidel na úrovni skupin
    %Sprava dat v karantene, sprava uzivatelu (kdo je zodpovedny za data z jake domeny?)
    %Možnost, aby skupiny pravidel mohly obsahovat další skupiny
    %možnost upload transformeru přes administraci (pouze pro uživatele ADM)
    %možnost dotáhnout ontologie ze zadaného URI
    %možnost seběhnout ovlivněné pipelines při smazání skupiny pravidel
    %možnost zastavit rerun pipeline
    %Zobrazovat pro skupinu pravidel seznam dotčených pipelines
    %Zobrazovat pro transformer seznam dotčených pipelines
    %Udělat drobečkovou navigaci klikatelnou
    %Umožnit filtrovat výpisy podle hodnot jednotlivých atributů
    %Použít hodnoty z polí description z DB jako hinty

%Ostatní

    %u pravidel (QA, DN) kontrolovat validitu při ukládání (pokus o spuštění na nějakém grafu, jestli SPARQL neselže na syntaktickou chybu)
    %automaticky získávat neznámé ontologie při příchodu dat
    %role Corrector - editace dat uživatelem
    %linkování na externí zdroje - zavést podporu pro spouštění (periodické/manuální) něčeho s jasně definovaným rozhraním (obdoba transformeru) s možností správy ve frontendu; viz meeting 17.6.
    %možnost připojit k vkládanému jmennému grafu srozumitelný popisek
    %vytvořit nějaký (více či méně user-friendly) nástroj pro nahrávání dat ze souboru; možnost rozdělení velkých souborů na menší grafy

%Vizualizace

    %Create GUI depicting quality scores/justifications, coloring results
    %Create GUI which will enable user to annotated the given SPARQL query - sample annotation: select source XY for var ?a

%Conceptual (?)

    %? dirty database -> internal queue of graphs to be processed, clean dataase -> staging area, create new clean database (raw data mart) populated periodically from the staging area (once per day, all records/ only the newest updates will be merged with the clean database records)
        %query execution will be done on top od the new clean database (raw data mart)
