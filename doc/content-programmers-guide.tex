...
Engine is implemented as multithreaded server. Multiple requests for both Input and Output Webservice can be handled at the same time. The data processing part processes named graphs sequentially, one at a time. Engine is implemented so that extending to parallel data procesing should be simple, however. 
...
The implementation is in classes \code{SoapMethodInsertExecutor} extending \code{SoapMethodExecutor} in a way similar to standard \code{DefaultHandler} for \code{SAXParser}. \code{SoapMethodExecutor} is part of \code{odcs-comlib} and filters out parts of the message that belong to the SOAP protocol. The main execution part of message processing is in the \code{InsertExecutor} class which subsequently takes input parameters and saves received data to the filesystem as files with suffix \code{-d.ttl}/\code{-d.rdf} for the payload data, suffix \code{-m.ttl}/\code{-m.rdf} for metadata and \code{-pvm.ttl}/\code{-pvm.rdf} for provenance metadata, depending on the serialization format (Turtle or RDF/XML). In case of success, it signalizes that a new named graph was accepted to pipeline service which will eventually run a transformer pipeline on the new data.
...
\begin{description}[style=nextline,font=\ttfamily]
  ...
  \item[OLDGRAPHSPREFIXED] The temporary prefix is removed from the processed graphs and the possible old versions of named graphs, now prefixed with temporary prefix, are removed.
...
\section{Implementation}
Data processing in Engine is driven by settings in the relational database which are managed from \FE. Engine logs its activity to a log file called odcs.engine.log in the working directory of Engine. In addition, there is a log file for each transformer instance where the respective transformer logs its activity. These transformer instance logs are in the working directory of the respective transformer.
...
Classes that cover implementation of pipeline service are \code{PipelineService} which handles processing of named graphs in different states, \code{PipelineGraphManipulator} for manipulation with named graphs (e.g. moving between databse instances) and \code{PipelineGraphStatus} for working with state data (e.g. loaded transformer instances for the processing pipelines). \code{PipelineGraphTransformerExecutor} called from \code{PipelineService} is responsible for running of transformers on named graphs and \code{TransformationContext} and \code{TransformationGraph} are implementations of context objects passed to transformers.
