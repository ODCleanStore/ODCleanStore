...
\begin{lstlisting}[caption=Quality Assessment rule constructor,lst:qualityAssessmentRule]
public QualityAssessmentRule (Integer id, Integer groupId, String filter, Double coefficient,
  String label, String description) 
\end{lstlisting}

The data filters are described with \code{SPARQL Where Clause}\footnote{http://www.w3.org/TR/rdf-sparql-query/\#rWhereClause} followed by optional \code{Group by Clause}\footnote{http://www.w3.org/TR/sparql11-query/\#rGroupClause} and \code{Having Clause}\footnote{http://www.w3.org/TR/sparql11-query/\#rHavingClause}.
...
After the ontology is stored in the database in a separate named graph its graph name can be passed to \code{compileOntologyToRules} method. All resources specified in the ontology are processed based on their \code{RDF.type} property.
...
The \code{compileOntologyToRules} expects a named graph of a name \code{ontologyGraphURI} to be present in the database. Its contents are interpreted as definition of an ontology and examined as such.
...
By default links are created between transformed graph and graphs in the clean database. If you want to interlink transformed graph with itself, you can do it by setting \\\code{object\_identification.link\_within\_graph} to true in the global configuration or adding \code{linkWithinGraph=true} to the particular transformer configuration. When activated, linker creates two linkage rules in Silk-LSL for each rule loaded from database. One rule is for linking transformed graph with clean database, another for linking with itself.
...
This \code{core} package contains the following important classes. 

The list contains neither the standard Wicket \code{ODCSWebFrontendApplication} and \code{ODCSWebFrontentSession} classes, nor thestandard \code{ODCSWebFrontendApplication.properties} file. The properties file contains custom validation messages and declarations of other string literals.
...
\begin{description}[style=nextline,font=\ttfamily]
	...
	\item[RedirectButton] A button to redirect to a different page, adjusted to suit the structure of the \WF application.
